...
function findProductIndex(product) {
  return inventory.findIndex((ob) => ob.name === product.toLowerCase());
}

function addProduct(objProduct) {
  const productIndex = findProductIndex(objProduct.name);
  if (productIndex >= 0) {
    //продукт є в інвентарі
    inventory[productIndex].quantity += objProduct.quantity;
    console.log(`${inventory[productIndex].name} quantity updated`);
  } else if (productIndex < 0) {
    inventory.push({
      name: objProduct.name.toLowerCase(),
      quantity: objProduct.quantity,
    });
    console.log(`${objProduct.name.toLowerCase()} added to inventory`);
  }
}

function removeProduct(product, nummber) {
  const productIndex = findProductIndex(product);
  if (productIndex >= 0) {
    if (inventory[productIndex].quantity > nummber) {
      inventory[productIndex].quantity -= nummber;
      console.log(
        `Remaining ${inventory[productIndex].name} pieces: ${inventory[productIndex].quantity}`,
      );
    } else if (inventory[productIndex].quantity == nummber) {
      inventory.splice(productIndex, 1);
      console.log(inventory);
    } else {
      console.log(
        `Not enough ${product.toLowerCase()} available, remaining pieces: ${inventory[productIndex].quantity}`,
      );
    }
  } else {
