...
function translatePigLatin(str) {
  let consonant = /^[^aeiou]+/i; //приголосні
  const found = str.match(consonant); // чи знайшли шось

  return found ? str.slice(found[0].length) + found + "ay" : str + "way"; //якшо так то обрізали, перенесли на зад. якщо ні то тільки додали в ніеці "way"
}

let test = "schwartz";

console.log(translatePigLatin(test)); // artzschway
